...
Mit dem OpenStack-Client können Sie das in der Konfigurationsdatei `clouds.yaml` angegebene Cloud-Profil festlegen, indem Sie den Parameter `--os-cloud <Cloud-Identifikator>` angeben.
...
Wenn Sie Ihre `openrc`-Datei haben, diese Informationen aber in einer `clouds.yaml`-Datei benötigen, kann dies leicht konvertiert werden.
...
# OpenStack-API Version 3 ist. OS_AUTH_URL gilt nur für die über Keystone bereitgestellte Identity API.
